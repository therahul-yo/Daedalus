        ...
        with self._condition:
            ...
            while ticket != self._serving:
                if abort_event and abort_event.is_set():
                    # Abandon our place in line. Only a release may move the
                    # cursor: advancing it from here while another caller
                    # still holds the lock would admit a second holder.
                    self._abandoned.add(ticket)
                    self._condition.notify_all()
                    return False
                self._condition.wait(timeout)
            if abort_event and abort_event.is_set():
                # Our turn arrived, but the client is already gone.
                self._advance()
                self._condition.notify_all()
                return False
            self._held = True
